...
public class FullFlowStatWeek {
    ...
    public static void getFullFlowResult(SparkSession session,String tdate,String start_date,String enddate){
        ...
        System.out.println("获取上一周的yyyy_MM_dd的时间是："+tdate_start+"|"+tdate_end);
        ...
        final HashMap<String,HashMap<String,String>> city_version_uploadcount=new HashMap<String,HashMap<String,String>>();
        ...
        JavaRDD<MacViewExtend> macextend = mac_view_data.repartition(600).toJavaRDD().mapPartitions(new FlatMapFunction<Iterator<Row>, MacViewExtend>() {
            @Override
        ...
        day_fullflow.write().mode(SaveMode.Append).jdbc(URL,"ad_playtimes_week",pro);
    }
    ...
    public static String getWeekDayUntil(String tdate){
        SimpleDateFormat datefotmat=new SimpleDateFormat("yyyy_MM_dd");
        SimpleDateFormat linefotmat=new SimpleDateFormat("yyyy-MM-dd");
        Calendar calendar=Calendar.getInstance();
        try {
            Date tdate_date=datefotmat.parse(tdate);
            calendar.setTime(tdate_date);
            //代表今天是星期几
            ...
            //上一周的截至时间
            Date end_week_time = calendar.getTime();

            calendar.setTime(end_week_time);

            calendar.add(Calendar.DAY_OF_WEEK,-6);
            Date start_week_timw=calendar.getTime();

            return linefotmat.format(start_week_timw)+"|"+linefotmat.format(end_week_time);
    ...
    public static String getTdateWeekDayUntil(String tdate){
        ...
        try {
            ...
            return datefotmat.format(start_week_timw)+"|"+datefotmat.format(end_week_time);
        } catch (ParseException e) {
            e.printStackTrace();
        }
        return null;
    ...
    public static void main(String[] args){
       // System.out.println(getWeekDayUntil("2018_06_01"));

        String tdate=args[0];

        SparkSession session = JavaSparkUtil.getRemoteSparkSession("week_full_flow", "parquet_table");

        String weekDayUntil = getWeekDayUntil(tdate);

        String[] split = weekDayUntil.split("\\|");
        System.out.println("获取的上一周的yyyy-MM-dd时间是:"+split[0]+"|"+split[1]);
