...
public class GenerateNewMacScanInfoZnp {
    ...
    public static void main(String[] args){
       ...
       // conf.set("spark.shuffle.sort.bypassMergeThreshold","800");
        ...
        Dataset<Row> fmlocation_new = spark.sql("select devid,suit_kind,city_name,building_no,building_name from parquet_table.fm_location_new where time='" + partitionDate.get(partitionDate.size() - 1) + "' and install_status='已安装' and devid !='' and devid is not null and suit_kind is not null and suit_kind !='' group by devid,suit_kind,city_name,building_no,building_name");
        ...
        Dataset<Row> schedule_adlength = spark.sql("select filelocation as adcontent,adcustomer,adproduct,max(cast(adlength as int)) as ad_length from ods_table.ods_schedule_fdm where time<='" + partitionDate.get(0) + "' and time>='" + partitionDate.get(partitionDate.size() - 1) + "' group by filelocation,adcustomer,adproduct");
    ...
    public static void one_getMacScaninfo(SparkSession session,String tdate,String next_tdate,long must_append_time,
        ...
        Dataset<Row> schedule_play = session.sql(" select d.devid,d.iccid,d.inner_sn,d.pid,d.st,d.et,d.buildinglist,d.pi,d.idx,d.adcontent,d.uploadcount " +
                " from  (select devid,iccid,inner_sn,pid,st,et,'0' as buildinglist,pi,idx,substr(trim(vfn),4) as adcontent,'0' as uploadcount " +
                " from ods_table.ods_playlog_day_znp where (time  like '%" + tdate + "%' or time like '%"+next_tdate+"%' )  and substr(st,1,8) in ('"+noline_date+"','"+noline_nextdate+"')  and vfn !='' and vfn is not null " +
                " group by devid,iccid,inner_sn,pid,st,et,sv,pi,idx,vfn ) d " +
        ...
        Dataset<Row> newschedule_play = session.sql(" select k.devid,k.iccid,k.inner_sn,k.pid,k.st,k.et,k.buildinglist,k.pi,k.idx,k.adcontent,k.uploadcount," +
                ...
                " from tmp_schedule_day g inner join tmp_fmlocation_new w on g.devid=w.devid ) k inner join tmp_schedule n on substr(trim(k.adcontent),1,length(trim(k.adcontent))-4) = substr(trim(n.adcontent),1,length(trim(n.adcontent))-4)  ");
        ...
        Dataset<Row> schedule_screen = session.sql("select h.devid,h.iccid,h.inner_sn,h.pid,h.st,h.et,h.buildinglist,h.pi,h.idx,h.adcontent,h.uploadcount," +
                " h.suit_kind,h.city_name,h.ad_length,h.adcustomer,h.adproduct ,j.adid as ad_content_id,h.building_no,h.building_name " +
                " from tmp_length_screen h inner join ods_table.ods_znp_adcontent_adid j on substr(trim(h.adcontent),1,length(trim(h.adcontent))-4) = substr(trim(j.adcontent),1,length(trim(j.adcontent))-4) ");
        ...
        macViewDataFrame.repartition(300).write().mode(SaveMode.Overwrite).parquet("/user/yarn/znp/adinfosys/"+tdate);
    ...
    * */
    ...
    public static void saveMacViewInfo(SparkSession session,String macViewTable,String tdate){
        ...
        session.sql("load data inpath '/user/yarn/znp/adinfosys/"+tdate+"/*.parquet' overwrite into table db01."+macViewTable+" partition (time='"+tdate+"')");
                ...
                try{
                    ...
                    if (cur_exposelog.scantimelong - pre_mac_viewlog.end_time_long > expose_flag_time) {
                        ...
                        if (cur_exposelog.getAd_length()!=null && cur_exposelog.sch_version.equals(pre_mac_viewlog.sch_version) &&
                                ...
                                cur_exposelog.adcontent.equals(pre_mac_viewlog.adcontent) &&
