...
public class GenerateNewMacScanInfoAdplaylog_test {
...
//                " ) t inner join (select devid from dw_table.dw_playlog_day where time like '%"+tdate+"%' group by devid ) k on t.devid = k.devid");
        ...
        Dataset<Row> schedule_play = session.sql(" select d.devid,d.iccid,d.inner_sn,d.pid,d.st,d.et,d.buildinglist,d.pi,d.idx,d.adcontent,d.uploadcount " +
                ...
                " from dw_table.dw_playlog_day where (time  = '" + tdate + "')  and substr(st,1,8) in ('"+noline_date+"')  and vfn !='' and vfn is not null " +
        ...
        int numPartitions = schedule_screen.javaRDD().getNumPartitions();
        System.out.println("===================================Schedule Partitions num is:"+ numPartitions);
        ...
        //session.sql("select * from tmp_schedule_day").show();
        JavaPairRDD<String, Tuple2<Iterable<String>, Iterable<String>>> cogroup = macStringRDD.cogroup(scheduleStringRDD);

        //对应union后的
        //JavaPairRDD<String, String> play_and_macview =  session.sparkContext().union(scheduleStringRDD);
        JavaRDD<MacResultBeanAD> macViewBeanJavaRDD = cogroup.flatMap(new FlatMapFunction<Tuple2<String,Tuple2<Iterable<String>,Iterable<String>>>, MacResultBeanAD>() {
            @Override
            public Iterator<MacResultBeanAD> call(Tuple2<String, Tuple2<Iterable<String>, Iterable<String>>> tuple2) throws Exception {
                ...
                //获取当前设备下的扫描记录和播放记录
                Iterable<String> src_macs = tuple2._2._1;
                Iterable<String> src_playlogs = tuple2._2._2;
                ...
                //用来转换对应的mac数据
                src_macs.forEach(new Consumer<String>() {
                    @Override
                ...
                //用来转换对应的监播数据
                src_playlogs.forEach(new Consumer<String>() {
                    @Override
                    public void accept(String s) {
                        String[] cols = s.split("\t",-1);
                        long stlong =0 ;
